...
// Global wall thickness for borewell walls.
...
class WaterParticle {
  // Now accepts a spawnAngle parameter.
  constructor(x, y, z, spawnAngle) {
    this.pos = createVector(x, y, z);
    // Compute horizontal velocity so that the particle moves inward from the wall.
    let horizontal = p5.Vector.mult(createVector(-cos(spawnAngle), -sin(spawnAngle), 0), random(2, 4));
  ...
  update() {
    // Update acceleration based on global reverse mode.
...
// Water Particles: Spawn from the borewell walls.
function spawnWaterParticles() {
  // Choose a random angle within the visible slice.
  let angle = random(radians(-sliceAngle / 2), radians(sliceAngle / 2));
  // Choose a random radius from within the wall thickness.
  ...
  if (random() < 0.3) {
    // Pass the spawnAngle so the particle's velocity is computed inward.
...
// Mud Particles: Spawn from the borewell walls (using wall thickness range).
